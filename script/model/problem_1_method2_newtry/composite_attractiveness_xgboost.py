...
def _stacking_blend(
    ...
    for train_idx, val_idx in gkf.split(x, y, groups=groups):
        ...
        neg_count = float((y_tr == 0).sum())
        pos_count = float((y_tr == 1).sum())
        xgb = XGBRegressor(
            n_estimators=400,
            max_depth=5,
            learning_rate=0.05,
...
def main() -> None:
    ...
    train_mask = (
        df["season"].isin(RANK_BASED_SEASONS).to_numpy(dtype=bool) & known_mask
    )
    if not train_mask.any():
        train_mask = np.ones(len(df), dtype=bool)

    group_labels = (
        df["season"].astype(str) + "_" + df["week"].astype(str)
    ...
    if {"season", "week"}.issubset(df.columns):
        weekly_counts = (
            df.groupby(["season", "week"]).size().rename("weekly_count")
        )
        base_weights = (
            df[["season", "week"]]
            .merge(
    ...
    sample_weights = base_weights[train_mask] * np.where(y_train == 1, pos_weight, 1.0)

    oof_en, oof_xgb, metrics, blend_weights = _stacking_blend(
        x_train, y_train, sample_weights, group_labels[train_mask], RANDOM_SEED
    )
