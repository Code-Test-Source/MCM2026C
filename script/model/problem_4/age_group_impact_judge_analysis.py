...
OUTPUT_SUMMARY_PATH = "../../../data/result/age_group_judge_impact_summary.csv"
...
CONTROL_COUNTRY_COL = "celebrity_homecountry/region_rank"
CONTROL_STATE_COL = "celebrity_homestate_rank"
...
def _build_age_group_labels(
    ...
    freq_map = series.value_counts(dropna=False).to_dict()
    return series, freq_map


def _build_region_label(df: pd.DataFrame) -> pd.Series:
    """
    与 feature_engineering.py/region_impact_judge_analysis.py 保持一致：美国细分到州，其他国家单独列出。
    """
    if CONTROL_COUNTRY_COL not in df.columns or CONTROL_STATE_COL not in df.columns:
        return pd.Series(["Unknown"] * len(df), index=df.index)
    country = df[CONTROL_COUNTRY_COL].fillna("Unknown").astype(str)
    state = df[CONTROL_STATE_COL].fillna("Unknown").astype(str)
    is_us = country.isin(US_NAMES)
    combined = np.where(is_us, "United States - " + state, country)
    return pd.Series(combined, index=df.index).fillna("Unknown")


def _build_control_matrix(df: pd.DataFrame) -> pd.DataFrame:
    control_cols: List[str] = []
    for col in [CONTROL_INDUSTRY_COL, CONTROL_SEASON_COL, CONTROL_WEEK_COL]:
        if col in df.columns:
    ...
    control_df["region_label_ctrl"] = _build_region_label(df)
...
def _direction_consistency(
    ...
    for season, group in df.groupby("season"):
        in_group = group[group[age_col] == age_group][judge_col]
        out_group = group[group[age_col] != age_group][judge_col]
        if len(in_group) < MIN_SEASON_GROUP_SIZE or len(out_group) < MIN_SEASON_GROUP_SIZE:
            continue
        diff = in_group.mean() - out_group.mean()
        if diff > 0:
            signs.append(1)
        elif diff < 0:
            signs.append(-1)
...
def main() -> None:
    ...
    _validate_columns(df, [AGE_GROUP_COL], "年龄组列")
    ...
    df["age_group_label"], freq_map = _build_age_group_labels(
        df,
    ...
    age_groups = df["age_group_label"].value_counts().index.tolist()
    for age_group in age_groups:
        group = df[df["age_group_label"] == age_group][judge_col]
        others = df[df["age_group_label"] != age_group][judge_col]
        ...
        # 控制其他因素后的回归检验（年龄组指示变量）
        age_flag = (df["age_group_label"] == age_group).astype(int)
        X = pd.concat([control_df, age_flag.rename("age_group_flag")], axis=1)
        ...
        direction, consistency = _direction_consistency(df, "age_group_label", judge_col, age_group)
    ...
    print("=== 年龄组对评委打分影响：量化分析摘要 ===")
    print(f"评委打分列: {judge_col}")
    print(f"年龄组合并阈值: {MIN_FREQ_RATIO} | 合并低频: {MERGE_RARE_CATEGORIES}")
    ...
    for _, row in result.iterrows():
        ...
        print(
            f"年龄组={age_group} | 结论={verdict} | 方向={direction} | 平均差={mean_diff} | p值(控制后)={p_value}"
        )
