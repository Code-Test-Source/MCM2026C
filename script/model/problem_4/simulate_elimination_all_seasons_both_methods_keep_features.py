...
INPUT_RANK_PATH = (
    "../../../data/processed/pr4/"
    "2026_MCM_Problem_C_Data_popularity_features_with_attractiveness_xgboost_pr4.csv"
)
INPUT_PERCENT_PATH = (
    "../../../data/processed/pr4/"
    "2026_MCM_Problem_C_Data_popularity_features_with_attractiveness_xgboost_percent_pr4.csv"
)       #这里的input文件只是新加了单独评委的原始分数据，其他和本来的没区别
...
def _compute_percent_method(
    ...
    for (season, week), group in df.groupby(["season", "week"], sort=True):
        ...
        fan_votes_relative = fan_base.clip(lower=0)
        fan_votes_percent = _to_percent(fan_votes_relative)
        combined = judge_percent + fan_votes_percent

        ranked = active.copy()
        ranked["judge_percent"] = judge_percent
        ranked["fan_votes_relative"] = fan_votes_relative
        ranked["combined_percent"] = combined
        ranked = ranked.sort_values(
            ["combined_percent", "fan_votes_relative", "judge_percent"],
            ...
            row_dict.update(
                {
                    ...
                    "fan_votes_relative": row["fan_votes_relative"],
                    "combined_percent": row["combined_percent"],
                    "predicted_eliminated_percent": bool(
                        row["predicted_eliminated_percent"]
                    ),
                }
            )
            score_rows.append(row_dict)
...
def main() -> None:
    ...
    score_df = pd.merge(
        ...
        how="outer",
        suffixes=("_rank", "_percent"),
    ).sort_values(["season", "week", "celebrity_name"])
    if "week_total_judge" not in score_df.columns:
        score_df["week_total_judge"] = score_df["week_total_judge_rank"].combine_first(
            score_df["week_total_judge_percent"]
        )
    if "week_total_judge_percent" in score_df.columns:
        score_df = score_df.drop(columns=["week_total_judge_percent"])
